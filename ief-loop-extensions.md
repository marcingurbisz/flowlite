...
Do exploratory tests using our local `runPerfTestApp`. Search for bugs, visual and performance issues.
Shortly document what you have tested as an item in `TODO.md`. If you find issues, add them as TODO items in `TODO.md` and fix them immediately when they are in scope for the current loop.
Tool preference order for exploratory tests:
- Browser automation against the live app. It gives the fastest interactive feedback, real rendered UI state, and lets you inspect the current page while still taking screenshots.
- Playwright CLI or MCP as a fallback.
